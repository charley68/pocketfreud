...
from modules.rate_limiter import password_reset_limiter
...
def register_routes(app):
    ...
    def debug_session():
        return jsonify({
            ...
            'current_user.id': getattr(current_user, 'id', None),
            'current_user.username': getattr(current_user, 'username', None),
    ...
    if not app.secret_key:
        print("[DEBUG] app.secret_key is not set! Flask sessions will not work.")
    else:
        print(f"[DEBUG] app.secret_key is set: {app.secret_key}")

    # User loader for Flask-Login
    @login_manager.user_loader
    def load_user(user_id):
        print(f"[DEBUG] load_user called with user_id={user_id}")
        from modules.db import get_user_by_id
        user = get_user_by_id(user_id)
        print(f"[DEBUG] get_user_by_id returned: {user}")
        if user:
            u = User(user['id'], user['username'])
            print(f"[DEBUG] User object created: id={u.id}, username={u.username}, get_id()={u.get_id()}")
            return u
        print("[DEBUG] No user found for user_id")
        return None
    # -------------------- CONTEXT PROCESSORS --------------------
    ...
    def home():
        ...
        print(f"[DEBUG] home: request.cookies={request.cookies}")
        if not current_user.is_authenticated:
            return render_template('index.html')
        username = getattr(current_user, "username", None) or session.get('username')
        user_id = getattr(current_user, "id", None)
        current_session = get_current_session(user_id)
        if not current_session:
            # If no session, log out and force re-login
        ...
        return render_template('home.html', username=username, current_session_name=current_session_name)
    ...
    def logout():
        from flask_login import logout_user
        print("[DEBUG] /logout route called. Logging out user and clearing session.")
        logout_user()
        session.clear()
        flash('You have been logged out.')
        return redirect(url_for('home'))  # home route will show index.html if not authenticated
    ...
    class User(UserMixin):
        def __init__(self, id, username):
            ...
            print(f"[DEBUG] User.__init__ called: id={id}, username={username}")
        def get_id(self):
            print(f"[DEBUG] User.get_id called, returning {self.id}")
            return str(self.id)
    ...
    def signin():
        import logging
        logger = logging.getLogger("pocketfreud.signin")
        logger.debug(f"[DEBUG] /signin route called. Method: {request.method}, session: {dict(session)}, cookies: {request.cookies}")
        if request.method == 'POST':
            email = request.form['email'].strip().lower()
            pwd = request.form['password']
            remember = request.form.get('remember') == 'yes'
            logger.debug(f"[DEBUG] Login attempt: email={email}, password={pwd}, remember={remember}")
            profile = get_user_profile(email)
            logger.debug(f"[DEBUG] Profile lookup result: {profile}")
            if not profile:
                logger.debug("[DEBUG] No account found with that email.")
                session['login_failed'] = "No account found with that email."
                logger.debug("[DEBUG] Redirecting to signin with flash: No account found.")
                return redirect(url_for('signin'))
            user_id = profile['id']
            ...
            logger.debug(f"[DEBUG] Unpacked profile: id={user_id}, username={username}, verified={verified}")
            if not verified:
                logger.debug("[DEBUG] Email not verified.")
                session['login_failed'] = f"Please verify your email before signing in. <a href='{url_for('resend_verification', email=email)}'>Resend verification email</a>."
                logger.debug("[DEBUG] Redirecting to signin with flash: Email not verified.")
                return redirect(url_for('signin'))
            password_check = check_password_hash(password, pwd)
            logger.debug(f"[DEBUG] Password check result: {password_check}")
            if password_check:
                logger.debug(f"[DEBUG] Password correct for user_id={user_id}, logging in.")
                load_user_settings(user_id)
                user = User(user_id, username)
                login_user(user, remember=remember)
                flash('Login successful!')
                logger.debug("[DEBUG] Redirecting to home with flash: Login successful!")
                return redirect(url_for('home'))
            else:
                logger.debug("[DEBUG] Invalid credentials.")
                session['login_failed'] = 'Invalid credentials.'
                logger.debug("[DEBUG] Redirecting to signin with flash: Invalid credentials.")
                return redirect(url_for('signin'))
        # Always check for login_failed in session and flash it
        logger.debug(f"[DEBUG] Before flashing login_failed, session: {dict(session)}")
        if 'login_failed' in session:
            logger.debug(f"[DEBUG] Flashing login_failed: {session['login_failed']}")
            flash(session.pop('login_failed'))
        logger.debug(f"[DEBUG] GET request to /signin, rendering signin.html. session: {dict(session)}, cookies: {request.cookies}")
    ...
    def forgot_password():
        if request.method == 'POST':
            ...
            # Log the attempt
            logging.info(f"Password reset attempt for email: {email} from IP: {client_ip}")
            
            # Check for honeypot field (bot detection)
            if request.form.get('nickname'):
                logging.warning(f"Bot detected in password reset from IP: {client_ip}, email: {email}")
                flash("Invalid form submission.")
                return redirect(url_for('forgot_password'))
            
            # Rate limiting check (3 attempts per 15 minutes per IP and per email)
            if not password_reset_limiter.is_allowed(f"ip:{client_ip}"):
                remaining_time = password_reset_limiter.get_remaining_time(f"ip:{client_ip}")
                logging.warning(f"Rate limit exceeded for IP: {client_ip}")
                flash(f"Too many reset attempts. Please try again in {remaining_time // 60} minutes.")
                return redirect(url_for('forgot_password'))
            
            if not password_reset_limiter.is_allowed(f"email:{email}"):
                remaining_time = password_reset_limiter.get_remaining_time(f"email:{email}")
                logging.warning(f"Rate limit exceeded for email: {email}")
                flash(f"Too many reset attempts for this email. Please try again in {remaining_time // 60} minutes.")
                return redirect(url_for('forgot_password'))
            
            # Validate CAPTCHA (skip in development for now)
            if os.getenv('ENV', '').lower() not in ['dev', 'development', 'test']:
                captcha_error = check_captcha(request)
                if captcha_error:
                    logging.warning(f"CAPTCHA validation failed for IP: {client_ip}, email: {email}")
                    return redirect(url_for('forgot_password'))
            
            # Validate email format
            if not is_valid_email(email):
                logging.warning(f"Invalid email format submitted: {email} from IP: {client_ip}")
                flash("Please enter a valid email address.")
                return redirect(url_for('forgot_password'))
            
            # Generate token and attempt to save
            token = secrets.token_urlsafe(32)
            user_exists = save_user_reset_token(email, token)
            
            # Only send email if user exists
            if user_exists:
                reset_link = url_for('reset_password', token=token, _external=True, _scheme='https')
                send_reset_email(email, reset_link)
                logging.info(f"Password reset email sent to registered user: {email}")
            else:
                logging.warning(f"Password reset attempted for non-existent email: {email} from IP: {client_ip}")
            
            # Always show the same message for security (timing attacks prevention)
    ...
    @login_required
    def therapy():
        # Use Flask-Login's current_user.id for user ID
        user_id = getattr(current_user, "id", None)
        current_session = get_current_session(user_id)
        ...
        messages = get_session_messages(user_id, sessionName) if sessionName else []
    ...
    def restore_chat_route():
        ...
        try:
            success = restore_chat(user_id, session_name)
            if success:
                return '', 204
            else:
    ...
    def change_persona():
        ...
        user_id = getattr(current_user, "id", None)
        update_session_persona(user_id, current_session_name, new_persona)
    ...
    def journal_calendar():
        user_id = getattr(current_user, "id", None)
        journal_dates = get_journal_dates(user_id)
    ...
    def get_chat_thread_route(session_name):
        ...
        try:
            thread = get_chat_thread(user_id, session_name)
    ...
    def signup_with_plan():
        if request.method == 'GET':
            ...
            if not plan or plan not in ['basic', 'professional', 'premium']:
                if plan:
    ...
    def require_login():
        ...
        public_paths = ['/', '/signin', '/signup_with_plan', '/subscribe', '/verify_email', '/resend_verification',
        ...
        if needs_auth and not current_user.is_authenticated and request.method != 'OPTIONS':
    ...
    def contact():
        if request.method == 'POST':
            ...
            # Check CAPTCHA and handle errors
            captcha_error = check_captcha(request)
            if captcha_error:
                if captcha_error == "no_response":
                    return jsonify({'status': 'error', 'message': 'Please complete the CAPTCHA.'}), 400
                elif captcha_error == "bot_detected":
                    return jsonify({'status': 'error', 'message': 'Invalid form submission.'}), 400
                elif captcha_error == "validation_failed":
                    return jsonify({'status': 'error', 'message': 'CAPTCHA validation failed. Please try again.'}), 400
